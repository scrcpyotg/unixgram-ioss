...
struct UGCurrentAccount: Codable, Identifiable {
    let id: String
    let email: String?
    let username: String
    let role: String?
...
struct UGProfileLink: Decodable, Hashable {
    let id: String?
    let platform: String?
    let url: String?
    let label: String?
...
struct UGProfileChannel: Decodable, Identifiable, Hashable {
    let id: String
    let handle: String?
    let name: String?
    let avatarUrl: String?
...
struct UGProfileMusic: Decodable, Encodable, Hashable {
    ...
    private enum CodingKeys: String, CodingKey {
        case title, artist, coverUrl, provider, durationMs, duration, externalId, id
        case previewUrl, audioUrl, externalUrl
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        if let ms = try c.decodeIfPresent(Int.self, forKey: .durationMs) {
            durationMs = ms
        } else if let seconds = try c.decodeIfPresent(Int.self, forKey: .duration) {
        ...
        previewUrl = try c.decodeIfPresent(String.self, forKey: .previewUrl)
            ?? c.decodeIfPresent(String.self, forKey: .audioUrl)
