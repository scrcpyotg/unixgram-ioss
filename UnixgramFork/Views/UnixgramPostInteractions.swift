...
struct UnixgramPostInteractionBar: View {
    ...
    init(
        ...
        openCommentsInitially: Bool = false
    ) {
        self.post = post
        ...
        _showComments = State(initialValue: openCommentsInitially)
    }
...
struct UnixgramCommentsView: View {
    ...
    @State private var replyingTo: UGPostComment?
        ...
        VStack(spacing: 8) {
            if let replyingTo {
                HStack(spacing: 8) {
                    ...
                    Text("Ответ @\(replyingTo.author.username)")
            ...
            HStack(alignment: .bottom, spacing: 10) {
                TextField(
                    replyingTo == nil ? "Написать комментарий…" : "Написать ответ…",
                    text: $text,
                    axis: .vertical
                )
                    .lineLimit(1...5)
    ...
    private func sendComment() async {
        ...
        let parentCommentID = target.map { $0.parentCommentId ?? $0.id }
        ...
        do {
            ...
            if let parentCommentID {
                var replies = repliesByCommentID[parentCommentID] ?? localReplies(for: parentCommentID)
                if !replies.contains(where: { $0.id == created.id }) {
    ...
    @MainActor
    private func beginReply(to comment: UGPostComment) {
        replyingTo = comment
        let mention = "@\(comment.author.username) "
