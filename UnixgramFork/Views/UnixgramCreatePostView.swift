...
struct UnixgramCreatePostView: View {
    ...
    init(
        ...
        communityHandle: String? = nil,
        communityAvatarURL: String? = nil,
        onPublished: (() -> Void)? = nil
    ) {
        self.communityId = communityId
        self.communityName = communityName
        self.communityHandle = communityHandle
        self.communityAvatarURL = communityAvatarURL
    ...
    private var header: some View {
        HStack {
            ...
            Text(communityId == nil ? "Новый пост" : "Пост в канал")
    ...
    private var authorRow: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(communityName ?? liveSession.currentUser?.displayName ?? liveSession.currentUser?.username ?? "Unixgram")
                    .font(.system(size: 16, weight: .bold))

                if let handle = communityHandle ?? liveSession.currentUser?.username {
                    Text("@\(handle)")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
    ...
    private var audienceRow: some View {
        HStack {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(communityId == nil ? "Публикация" : "Публикация в канал")
                    .font(.system(size: 15, weight: .semibold))
                Text(communityId == nil ? "Обычный пост Unixgram" : (communityName ?? "Ваш канал"))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
    ...
    private var channelAvatar: some View {
        if let raw = communityAvatarURL, let url = URL(string: raw) {
            AsyncImage(url: url) { phase in
                switch phase {
                case .success(let image): image.resizable().scaledToFill()
                default: Circle().fill(Color.purple.opacity(0.20))
                }
            }
        } else {
            Circle()
