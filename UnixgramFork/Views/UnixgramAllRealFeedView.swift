...
struct UnixgramAllRealFeedView: View {
    ...
    private func loadMore() async {
        ...
        do {
            let result = try await UnixgramRealAPIClient.shared.realFeed(limit: 15, cursor: cursor)
            let existing = Set(store.feed.map(\.id))
            store.feed.append(contentsOf: result.feed.filter { !existing.contains($0.id) })
            store.feedCursor = result.nextCursor
        } catch {
