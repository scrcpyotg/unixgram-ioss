...
struct UnixgramPublicProfileView: View {
    ...
    var body: some View {
        ...
        .task {
            ...
            await content.load(tab: .stories, username: username)
        }
        .refreshable {
            await content.refresh(username: username, selectedTab: .posts)
            await content.load(tab: .stories, username: username)
    ...
    private var publicStoriesStrip: some View {
                ...
                ForEach(content.stories) { story in
                    Button {
                        ...
                        ZStack {
                            ...
                            if let raw = story.thumbnailUrl ?? story.previewUrl ?? story.imageUrl,
                               let url = URL(string: raw) {
                                ...
                                Image(systemName: story.videoUrl != nil ? "play.fill" : "sparkles")
                                    .foregroundStyle(.secondary)
                            }

                            if story.viewedByViewer != true {
                                RoundedRectangle(cornerRadius: 16)
                                    .stroke(
    ...
    private func profileCover(_ raw: String?) -> some View {
        if raw != nil {
            UnixgramAnimatedProfileMedia(rawURL: raw, contentMode: .fill)
        } else {
            LinearGradient(
                ...
                VStack(alignment: .leading, spacing: 2) {
                    if let handle = post.author?.username, !handle.isEmpty {
                        NavigationLink {
                            ...
                            Text(post.author?.displayName ?? handle)
