            ...
            VStack(alignment: .leading, spacing: 13) {
                ...
                HStack(spacing: 7) {
                    ...
                    Text(profile?.isOnline == false ? lastSeen(profile?.lastSeenAt) : "В сети")
                        .foregroundStyle(profile?.isOnline == false ? .secondary : .green)
                }
                .font(.system(size: 16))
...
private struct ProfileMusicStrip: View {
    ...
    private var displayText: String {
        ...
        return !title.isEmpty ? title : (!artist.isEmpty ? artist : "Музыка профиля")
    }
}
...
private struct ProfilePostCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
                ...
                HStack(spacing: 10) {
                    if let raw = music.coverUrl, let url = URL(string: raw) {
                        AsyncImage(url: url) { phase in
                            switch phase {
                            case .success(let image): image.resizable().scaledToFill()
                            default: RoundedRectangle(cornerRadius: 9).fill(Color.white.opacity(0.06))
                            }
                        }
            ...
            HStack(spacing: 18) {
                metric("heart", post.likesCount)
                metric("bubble.left", post.commentsCount)
                metric("arrow.2.squarepath", post.repostsCount)
                metric("eye", post.viewsCount)
                Spacer()
                metric("bookmark", post.bookmarksCount)
            }
        }
        .padding(14)
    ...
    private var postMedia: some View {
        let images = [post.imageUrl].compactMap { $0 } + (post.imageUrls ?? [])
...
private struct ProfileStoryViewer: View {
    ...
    var body: some View {
        ZStack {
            ...
            if let raw = story.imageUrl ?? story.previewUrl ?? story.thumbnailUrl,
               let url = URL(string: raw) {
                ...
                Image(systemName: story.videoUrl != nil ? "play.circle.fill" : "sparkles")
                    .font(.system(size: 68))
                    .foregroundStyle(.secondary)
            }
